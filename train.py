...
class RNNModel():
    ...
    def __init__(self, BATCH_SIZE, HIDDEN_SIZE, HIDDEN_LAYERS, VOCAB_SIZE,TIME_STEPS,BATCH_NUMS, learning_rate):
        ...
        with tf.name_scope('loss'):
            self.loss = sequence_loss_by_example([logits], [tf.reshape(self.targets, [-1])],
...
if __name__ =="__main__":
    ...
    with open('./dataset/天龙八部.txt', 'r', encoding='utf-8') as f:
        ...
        # 训练模型
        model = RNNModel(BATCH_SIZE, HIDDEN_SIZE, HIDDEN_LAYERS, VOCAB_SIZE,TIME_STEPS,BATCH_NUMS,learning_rate)
        print(model)
